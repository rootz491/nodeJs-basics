...
const initPayment = function(amount) {
  return new Promise((resolve, reject) => {
    ...
    PaytmChecksum.generateSignature(paymentObj, config.PAYTM_MERCHANT_KEY)
      .then(result => {
...
const responsePayment = function(paymentObject) {
  return new Promise((resolve, reject) => {
    if (
      PaytmChecksum.verifySignature(paymentObject, config.PAYTM_MERCHANT_KEY, paymentObject.CHECKSUMHASH)
    ) {
